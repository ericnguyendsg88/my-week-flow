...
import { supabase, DEVICE_ID } from "./supabase";
import { CalEvent } from "@/types/event";
import { CaptureItem } from "@/types/event";
...
function eventToRow(e: CalEvent): DbEvent {
  return {
    id: e.id,
    device_id: DEVICE_ID,
    title: e.title,
...
export async function pushEvents(events: CalEvent[]) {
...
export async function deleteEvent(id: string) {
  const { error } = await supabase.from("events").delete().eq("id", id).eq("device_id", DEVICE_ID);
  if (error) { setStatus("error", error.message); console.warn("[sync] deleteEvent:", error.message); }
}

export async function pullEvents(): Promise<CalEvent[] | null> {
  setStatus("syncing");
  const { data, error } = await supabase.from("events").select("*").eq("device_id", DEVICE_ID);
...
function captureToRow(c: CaptureItem): DbCapture {
  return {
    id: c.id,
    device_id: DEVICE_ID,
    kind: c.kind,
...
export async function pushCaptures(captures: CaptureItem[]) {
  if (captures.length === 0) return;
  const rows = captures.map(captureToRow);
...
export async function deleteCapture(id: string) {
  const { error } = await supabase.from("captures").delete().eq("id", id).eq("device_id", DEVICE_ID);
  if (error) console.warn("[sync] deleteCapture:", error.message);
}

export async function pullCaptures(): Promise<CaptureItem[] | null> {
  const { data, error } = await supabase.from("captures").select("*").eq("device_id", DEVICE_ID);
